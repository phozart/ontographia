...
// Map route-level views to internal view IDs
const VIEW_MAP = {
 ...
 */
export default function CapWorkspace({ view: routeView }) {
  ...
  useEffect(() => {
    ...
    // First check for route-level view (from URL path like /cap/services)
    // Then check query parameter (from internal navigation like ?view=service_catalog)
    let viewFromUrl = router.query.view;

    // Map route-level view to internal view ID
    if (routeView && VIEW_MAP[routeView]) {
      viewFromUrl = VIEW_MAP[routeView];
    }
  ...
  }, [router.isReady, router.query.view, routeView, urlInitialized, findGroupForView, expandedGroups]);
  ...
  const renderContent = () => {
    ...
    if (isCapView) {
      ...
      const artefacts = currentArtefacts || [];
      return (
            ...
            <div className={styles.headerLeft}>
              ...
              <div className={styles.headerInfo}>
                <div className={styles.headerTitle}>
                  ...
                  <span className={styles.headerCount}>{artefacts.length}</span>
                </div>
