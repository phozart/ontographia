...
export function parseSpaceParams(query) {
  ...
  // Parse optional params (domainId, projectId, or sessionId)
  if (params.length > 0) {
  ...
  if (params.length > 1 && result.domainId) {
    if (isValidProjectDisplayId(params[1])) {
      ...
      return result; // Invalid project ID format
    }
  }
 ...
 * @param {string} [domainId] - Domain display ID (optional)
 * @param {string} [projectId] - Project display ID (optional)
 * @returns {string} URL path
 */
export function buildSpaceUrl(space, view = null, domainId = null, projectId = null) {
  const spaceConfig = getSpace(space);
  if (!spaceConfig) {
    return '/app/spaces';
  }
