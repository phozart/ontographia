 ...
 * Checks Bearer token first, then falls back to x-user/x-role headers.
 */
export function getUserFromRequest(req) {
  // 1. Try Bearer token (JWT from login endpoint)
  ...
  if (token) {
    ...
    if (result.valid && result.payload) {
      return {
        user: result.payload.username || result.payload.sub,
        role: result.payload.role || null,
      };
    }
  }

  // 2. Fall back to x-user/x-role headers (client sends these from session)
  return {
...
export async function getAccessibleProjects(userId, systemRole, domainId = null) {
  try {
    ...
    if (result.rows.length === 0 && totalResult.rows[0].count > 0) {
      ...
      const allProjects = await query('SELECT id, name, domain_id FROM projects LIMIT 5');
    ...
    if (err.code === '42P01' && err.message.includes('project_members')) {
      console.log('[getAccessibleProjects] Creating missing project_members table...');
      try {
        await query(`
        ...
        console.log('[getAccessibleProjects] Table created, retrying query...');
        // Retry the query after creating the table
        return getAccessibleProjects(userId, systemRole, domainId);
      } catch (createErr) {
        console.error('[getAccessibleProjects] Failed to create table:', createErr);
      }
    }
