...
export const PDW_TYPE_DEFS = {
  ...
  pdw_idea: {
    ...
    fields: {
      ...
      description: { type: 'textarea', required: false, label: 'Description', placeholder: 'Add details later...' },
...
export const PDW_CANVAS_DEFS = {
  ...
  // ---------------------------------------------------------------------------
  // USER-DEFINED CUSTOM CANVAS
  // ---------------------------------------------------------------------------
  pdw_custom_canvas: {
    id: 'pdw_custom_canvas',
    name: 'Custom Canvas',
    category: 'canvas',
    module: 'business', // Default module, but user-defined
    color: '#8b5cf6',
    icon: 'Build',
    description: 'User-defined canvas with custom sections',
    allowMultiple: true,
    fields: {
      name: { type: 'text', required: true, label: 'Canvas Name' },
      // Custom fields are stored in canvas_definition within custom_fields
      // The canvas_definition contains: { name, color, icon, description, fields: [{key, label, color, placeholder}] }
    },
    layout: 'custom',
  },
};
