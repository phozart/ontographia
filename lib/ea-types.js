...
// ============================================================================
// ADR (Architecture Decision Record) STATUS ENUM
// ============================================================================
...
export const ADR_STATUS_CONFIG = {
  [ADR_STATUS.PROPOSED]: {
    label: 'Proposed',
    color: '#f59e0b',
    bgColor: 'rgba(245, 158, 11, 0.1)',
    icon: 'HourglassEmpty',
    description: 'Decision is under review'
  },
  [ADR_STATUS.ACCEPTED]: {
    label: 'Accepted',
    color: '#22c55e',
  ...
  [ADR_STATUS.DEPRECATED]: {
    ...
    icon: 'Cancel',
    description: 'Decision is no longer recommended'
  },
  [ADR_STATUS.SUPERSEDED]: {
    ...
    icon: 'SwapHoriz',
    description: 'Decision has been replaced by a newer one'
  }
};

// ============================================================================
// ADR ARTEFACT TYPE DEFINITION
// ============================================================================

export const ADR_ARTEFACT_TYPE = {
  id: 'architectureDecisionRecord',
  ...
  color: '#7c3aed',
  icon: 'Gavel',
  description: 'A documented architecture decision with context, rationale, and consequences',
  ...
  guidance: {
    ...
    whenNotToUse: [
      'For minor technical choices (use code comments)',
      'For temporary decisions (use meeting notes)',
      'For requirements (use Requirement type)',
      'For project tasks (use Work Package)'
    ],
    bestPractices: [
      'Write ADRs at the time of decision, not after',
      'Keep context focused on the specific problem',
      'Document alternatives considered, not just the choice',
      'Include both positive and negative consequences',
      'Link related ADRs for decision chains',
      'Review ADRs periodically for relevance'
    ],
    antiPatterns: [
      { pattern: 'Writing ADRs after the fact', fix: 'Document decisions when made, while context is fresh' },
      { pattern: 'Vague context', fix: 'Be specific about the problem and constraints' },
      { pattern: 'Missing consequences', fix: 'Always document both benefits and drawbacks' },
      { pattern: 'No alternatives', fix: 'Document at least 2-3 alternatives considered' },
      { pattern: 'Orphan ADRs', fix: 'Link to related elements, decisions, and stakeholders' }
    ],
    examples: {
      good: [
        'ADR-001: Use PostgreSQL for primary data store',
        'ADR-015: Adopt microservices architecture for order processing',
        'ADR-023: Implement event sourcing for audit trail'
      ],
      bad: [
    ...
    togafPhase: 'All phases - decisions are made throughout the ADM cycle',
...
[What becomes easier or more difficult to do because of this change?]`
  ...
  fields: {
    title: {
      type: 'text',
      required: true,
      helpText: 'A short, descriptive title for the decision'
    },
    status: {
      type: 'select',
      required: true,
      options: Object.values(ADR_STATUS),
      helpText: 'Current status of the decision'
    },
    context: {
      type: 'textarea',
      required: true,
      helpText: 'What is the issue motivating this decision? Include constraints and forces.'
    },
    decision: {
      type: 'textarea',
      required: true,
      helpText: 'What is the change being proposed or implemented?'
    },
    consequences: {
      type: 'textarea',
      required: true,
      helpText: 'What becomes easier or harder? Include both positive and negative outcomes.'
    },
    rationale: {
      type: 'textarea',
      helpText: 'Why was this option chosen over alternatives?'
    },
    alternatives: {
      type: 'array',
      helpText: 'Other options that were considered'
    },
    deciders: {
      type: 'array',
      helpText: 'People who made or approved this decision'
    },
    dateDecided: {
      type: 'date',
      helpText: 'When the decision was made'
    },
    dateSuperseded: {
      type: 'date',
      helpText: 'When this decision was superseded (if applicable)'
    },
    supersededBy: {
      type: 'reference',
      helpText: 'Reference to the ADR that supersedes this one'
    },
    relatedArtefacts: {
      type: 'array',
      helpText: 'Related EA elements, capabilities, or other artefacts'
    },
    relatedStandards: {
      type: 'array',
      helpText: 'Related EA standards'
    }
  }
};

// ============================================================================
// EA ARTEFACT TYPES (high-level categorization)
// ============================================================================

export const EA_ARTEFACT_TYPES = {
  architectureDecisionRecord: ADR_ARTEFACT_TYPE,
  // Other artefact types can be added here
};
