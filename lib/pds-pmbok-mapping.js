...
/**
 * SRS Integration: Decision-to-Project Readiness
 * Validates that SRS decision is ready for project initiation
 */
export const SRS_READINESS_CRITERIA = {
  requiredScore: 70,
  requiredStatus: 'decided',
  minimumAlternatives: 3,

  /**
   * Validate SRS decision for project creation
   * @param {Object} decision - The SRS decision to validate
   * @returns {Object} Validation result with isReady and issues
   */
  validate: (decision) => {
    const issues = [];

    if (!decision) {
      return { isReady: false, issues: ['No SRS decision provided'] };
    }

    // Check readiness score
    if ((decision.readiness_score || 0) < SRS_READINESS_CRITERIA.requiredScore) {
      issues.push(`Readiness score (${decision.readiness_score || 0}%) is below required ${SRS_READINESS_CRITERIA.requiredScore}%`);
    }

    // Check decision status
    if (decision.status !== SRS_READINESS_CRITERIA.requiredStatus) {
      issues.push(`Decision status is "${decision.status || 'unknown'}" but must be "${SRS_READINESS_CRITERIA.requiredStatus}"`);
    }

    // Check alternatives count
    const altCount = decision.alternatives?.length || decision.custom_fields?.alternatives_evaluated || 0;
    if (altCount < SRS_READINESS_CRITERIA.minimumAlternatives) {
      issues.push(`Only ${altCount} alternatives evaluated, minimum ${SRS_READINESS_CRITERIA.minimumAlternatives} required`);
    }

    return {
      isReady: issues.length === 0,
      issues,
      score: decision.readiness_score || 0,
...
export const CROSS_STUDIO_RELATIONSHIPS = {
  ...
  srs_to_pds: [
    {
      fromType: 'srs_decision',
      toType: 'pds_project',
      relationshipType: 'originated_from',
      description: 'Project originated from SRS decision',
      required: true,
    },
    {
      fromType: 'srs_assumption',
      toType: 'pds_assumption',
      relationshipType: 'imported_from',
      description: 'Assumption carried forward from SRS',
      required: false,
    },
    {
      fromType: 'srs_stakeholder',
      toType: 'pds_stakeholder',
      relationshipType: 'imported_from',
      description: 'Stakeholder identified in SRS',
      required: false,
    },
  ],
