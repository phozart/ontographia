...
import { getPipelineStage, DISPLAY_ID_PREFIXES } from '../pipeline-types';
...
export class WorkspaceArtefactRepository extends BaseRepository {
  ...
  async createArtefact(data) {
    ...
    // Auto-assign pipeline_stage from artefact type
    const pipelineStage = getPipelineStage(artefactType);

    // Generate display_id for work breakdown types (REQ-001, EPIC-001, etc.)
    let displayId = null;
    const baseType = artefactType.replace(/^[a-z]+_/, ''); // strip prefix like "pdw_"
    const seqMap = {
      requirement: 'seq_requirement',
      epic: 'seq_epic',
      feature: 'seq_feature',
      user_story: 'seq_story',
      test_case: 'seq_test_case',
      test_suite: 'seq_test_suite',
      test_run: 'seq_test_run',
    };
    const prefix = DISPLAY_ID_PREFIXES[baseType];
    const seqName = seqMap[baseType];
    if (prefix && seqName) {
      try {
        const seqResult = await query(`SELECT nextval('${seqName}') as num`);
        displayId = `${prefix}-${String(seqResult.rows[0].num).padStart(3, '0')}`;
      } catch {
        // Sequence may not exist yet — skip display_id
      }
    }
    ...
    const result = await query(
      `INSERT INTO artefacts (
        ...
        pipeline_stage, display_id,
        created_by, created_at, updated_at
      ) VALUES ($1, $2, $3, $4, $5, 'N/A', 'Medium', $6, $7, $8, $9, $10, $11, now(), now())
      RETURNING *`,
      [
        projectId,
        artefactType,
