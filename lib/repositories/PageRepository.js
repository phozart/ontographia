...
export class PageRepository extends BaseRepository {
  ...
  async getUserAccessiblePages(userId) {
    // Get user's role - support both username and UUID lookup
    const userResult = await query(
      'SELECT id, username, role FROM users WHERE username = $1 OR id::text = $1',
      [userId]
    );
    ...
    const userIdentifier = userResult.rows[0].username || userId;
    ...
    const result = await query(sql, [userIdentifier, userRole]);
