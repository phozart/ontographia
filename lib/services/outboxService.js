...
export const OUTBOX_ACTIONS = {
  ...
  // Graph materialisation actions
  MATERIALISE_ARTEFACT: 'materialise_artefact',
  DEMATERIALISE_ARTEFACT: 'dematerialise_artefact',
...
async function processOneEvent(event) {
  ...
  switch (action) {
    ...
    // ── Graph materialisation actions ─────────────────────────────
    case OUTBOX_ACTIONS.MATERIALISE_ARTEFACT: {
      const { materialiseArtefact } = await import('./graphMaterialisation');
      await materialiseArtefact(payload);
      break;
    }
    case OUTBOX_ACTIONS.DEMATERIALISE_ARTEFACT: {
      const { dematerialiseArtefact } = await import('./graphMaterialisation');
      await dematerialiseArtefact(payload.artefactId);
      break;
    }
