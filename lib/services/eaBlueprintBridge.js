...
import { EVENT_TYPES } from './innovationEvents';
import { enqueueOutboxEvents, OUTBOX_ACTIONS } from './outboxService';
...
export async function createInitiativeFromCapabilityGap(capabilityGap, domainId, actor = null) {
  ...
  ]).catch(err => console.error('[EABlueprintBridge] Failed to enqueue outbox:', err.message));
