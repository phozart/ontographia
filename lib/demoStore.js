...
export function listNodes({ typeIds = [], domain, domains } = {}) {
  ...
  return nodes
    ...
    .filter(n => {
      if (!requestedDomains.size) return true;
      const t = nodeTypes.find(nt => nt.id === n.typeId);
      const nodeDomain = n.domain || t?.domain || 'core';
      return requestedDomains.has(String(nodeDomain));
    })
    .map(n => {
      const t = nodeTypes.find(nt => nt.id === n.typeId);
      return {
...
export function listRelationships(nodeId, domain) {
  const nodeMatchesDomain = id => {
    ...
    const n = getNode(id);
    const nodeDomain = n?.domain || 'core';
    return String(nodeDomain) === String(domain) || (!n?.domain && String(domain) === 'core');
  };
