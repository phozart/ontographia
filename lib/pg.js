...
async function initSchema() {
  ...
  initPromise = (async () => {
    ...
    try {
      ...
      // ============ OUTBOX EVENTS TABLE ============
      // Transactional outbox for reliable cross-studio sync to the knowledge graph
      await client.query(`
      ...
      `);
      await client.query(`CREATE INDEX IF NOT EXISTS idx_outbox_status ON outbox_events(status, created_at)`);
      await client.query(`CREATE INDEX IF NOT EXISTS idx_outbox_aggregate ON outbox_events(aggregate_type, aggregate_id)`);
