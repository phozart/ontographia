  ...
  initPromise = (async () => {
    ...
    try {
      ...
      await client.query(`
        ...
        END $$;
      `);
      await client.query(`CREATE INDEX IF NOT EXISTS idx_srs_sessions_project ON srs_sessions(project_id);`);
      ...
      // Migration: Fix project_id type if it was created as INTEGER
      await client.query(`
        DO $$
        BEGIN
          IF EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_name = 'srs_sessions' AND column_name = 'project_id' AND data_type = 'integer'
          ) THEN
            ALTER TABLE srs_sessions ALTER COLUMN project_id TYPE UUID USING NULL;
          END IF;
        END $$;
      `);
      ...
      await client.query(`
        INSERT INTO menu_items (key, label, href, icon, default_section, roles, sort_order) VALUES
          -- Navigation section
          ('projects-overview', 'Projects Overview', '/navigation/projects-overview', 'DashboardIcon', 'navigation', '{admin,editor,viewer}', 0),
          ('product', 'Product', '/navigation/home', 'AppsIcon', 'navigation', '{admin,editor,viewer}', 1),
          ('login', 'Login', '/login', 'LockIcon', 'navigation', '{}', 2),
          -- Knowledge section
          ('knowledge-studio', 'Knowledge Studio', '/app/knowledge/studio', 'HubIcon', 'knowledge', '{admin,editor,viewer}', 0),
          -- Workspace section
          ('organisation-studio', 'Organisation Studio', '/app/workspaces/organisation', 'BusinessIcon', 'workspace', '{admin,editor,viewer}', 0),
          ('business-service-studio', 'Service Management', '/business-service-studio', 'MiscellaneousServicesIcon', 'workspace', '{admin,editor,viewer}', 1),
          ('performance-studio', 'Performance Studio', '/performance-studio', 'FlagIcon', 'workspace', '{admin,editor,viewer}', 2),
          ('governance-studio', 'Governance Studio', '/governance-studio', 'GavelIcon', 'workspace', '{admin,editor,viewer}', 3),
          ('risk-studio', 'Risk & Resilience', '/risk-studio', 'ShieldIcon', 'workspace', '{admin,editor,viewer}', 4),
          ('product-design', 'Product Design', '/app/workspaces/product-design', 'LightbulbIcon', 'workspace', '{admin,editor,viewer}', 5),
          ('requirements-studio', 'Requirements Studio', '/app/workspaces/requirements', 'AssignmentIcon', 'workspace', '{admin,editor,viewer}', 6),
          ('enterprise-architecture', 'Enterprise Architecture', '/app/workspaces/enterprise-architecture', 'ArchitectureIcon', 'workspace', '{admin,editor,viewer}', 7),
          ('diagram-studio', 'Diagram Studio', '/diagram-studio-standalone', 'GridViewIcon', 'workspace', '{admin,editor,viewer}', 8),
          ('change-studio', 'Change Studio', '/app/workspaces/change-management', 'ChangeCircleIcon', 'workspace', '{admin,editor,viewer}', 9),
          -- Reasoning section
          ('system-dynamics', 'System Dynamics', '/app/reasoning/system-dynamics', 'LoopIcon', 'reasoning', '{admin,editor,viewer}', 0),
          ('work-design', 'Work Design', '/app/reasoning/dynamic-work-design', 'BuildIcon', 'reasoning', '{admin,editor,viewer}', 1),
          ('negotiation', 'N&P Studio', '/app/reasoning/negotiation', 'HandshakeIcon', 'reasoning', '{admin,editor,viewer}', 2),
          ('sensemaking', 'Sensemaking', '/app/reasoning/sensemaking', 'PsychologyIcon', 'reasoning', '{admin,editor,viewer}', 3),
          ('learning', 'Learning Studio', '/app/reasoning/learning', 'SchoolIcon', 'reasoning', '{admin,editor,viewer}', 4),
          ('philosophy', 'Philosophy', '/philosophy-studio', 'AutoStoriesIcon', 'reasoning', '{admin,editor,viewer}', 5),
          ('strategic-reasoning', 'Strategic Reasoning', '/strategic-reasoning', 'AutoGraphIcon', 'reasoning', '{admin,editor,viewer}', 6)
        ON CONFLICT (key) DO NOTHING;
      `);
      ...
      await client.query(`
        ...
        VALUES ('portfolio-studio', 'Portfolio Studio', '/app/workspaces/portfolio', 'BusinessCenterIcon', 'workspace', '{admin,editor,viewer}', 10)
        ON CONFLICT (key) DO NOTHING;
      `);

      // Project Design Studio - added for PDS workspace
      await client.query(`
        INSERT INTO menu_items (key, label, href, icon, default_section, roles, sort_order)
        VALUES ('project-design', 'Project Design', '/app/workspaces/project-design', 'AccountTreeIcon', 'workspace', '{admin,editor,viewer}', 0)
      ...
      // Also update existing organisation-studio href if it's pointing to old path
      await client.query(`
        UPDATE menu_items
        SET href = '/app/workspaces/organisation'
        WHERE key = 'organisation-studio' AND href = '/organisation-studio';
      `);
        ...
        sections: [
          {
            ...
            expanded: true,
            items: [
              { key: 'projects-overview', label: 'Projects Overview' },
          ...
          },
          {
            ...
            items: [
              ...
              { key: 'business-service-studio', label: 'Service Management' },
              { key: 'performance-studio', label: 'Performance Studio' },
              { key: 'governance-studio', label: 'Governance Studio' },
              { key: 'risk-studio', label: 'Risk & Resilience' },
          ...
          },
          {
            ...
            items: [
              ...
              { key: 'negotiation', label: 'N&P Studio' },
              { key: 'sensemaking', label: 'Sensemaking' },
              { key: 'learning', label: 'Learning Studio' },
              { key: 'philosophy', label: 'Philosophy' },
              { key: 'strategic-reasoning', label: 'Strategic Reasoning' }
            ]
          }
        ]
      })]);
      ...
      await client.query(`
        INSERT INTO page_registry (path, name, category, section, description, default_roles, sort_order) VALUES
          ...
          ('/navigation/projects-overview', 'Projects', 'Navigation', 'navigation', 'Projects overview', '{admin,editor,viewer}', 11),
          ('/app/knowledge/studio', 'Knowledge Studio', 'Knowledge', 'knowledge', 'Knowledge graph studio', '{admin,editor,viewer}', 20),
          ('/app/knowledge/studio/graph-navigator', 'Graph Navigator', 'Knowledge', 'knowledge', 'Navigate the knowledge graph', '{admin,editor,viewer}', 21),
