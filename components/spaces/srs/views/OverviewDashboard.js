 ...
 *
 * Shows recent sessions at top, then active session progress,
 * with journey explanation collapsed by default for returning users.
 */
...
export default function OverviewDashboard({ onStartWizard, onStartDemo }) {
  const {
    sessions,
    activeSession,
    loadSession,
  ...
  // Collapsible sections - collapsed by default if user has sessions
  const [showJourneyGuide, setShowJourneyGuide] = useState(sessions.length === 0);
  ...
  // Note: Recent sessions are now handled by RecentSessionsRail component
  ...
  return (
    <div className="srs-overview-wrapper">
      {/* Recent Sessions Rail - Always at top if there are sessions */}
      {sessions.length > 0 && (
        <RecentSessionsRail onCreateSession={handleCreateSession} />
      )}

      {/* First-time user welcome - only when no sessions */}
      {sessions.length === 0 && (
        <div className="srs-overview__welcome srs-overview__welcome--first">
          ...
          <div className="srs-welcome-action">
            ...
            <p className="srs-welcome-hint">
              The guided wizard walks you through each reasoning phase with prompting questions
            </p>
            {onStartDemo && (
              <div className="srs-welcome-demo">
                <button className="srs-btn srs-btn--demo" onClick={onStartDemo}>
                  <SchoolIcon />
                  Learn with Demo Example
                </button>
                <p className="srs-welcome-demo-hint">
                  Explore a complete example to see how strategic reasoning works
                </p>
              </div>
            )}
          </div>
      ...
      {/* Journey Guide - Collapsible for returning users */}
      <div className="srs-journey-section">
        <button
          className="srs-journey-toggle"
          onClick={() => setShowJourneyGuide(!showJourneyGuide)}
        >
          <span>The Reasoning Journey</span>
          {showJourneyGuide ? <ExpandLessIcon /> : <ExpandMoreIcon />}
        </button>

        {showJourneyGuide && (
          <div className="srs-journey-content">
        ...
        <>
          ...
          {sessionProgress?.suggestedNext && (
            <div className="srs-suggested-next">
              ...
              <div className="srs-suggested-actions">
                <button
                ...
                {onStartWizard && (
                  <button
                    className="srs-btn srs-btn--wizard-outline"
                    onClick={onStartWizard}
                    title="Use guided prompts to build your reasoning"
                  >
      ...
      {/* Session management is now handled by the rail at the top */}
      ...
      <style jsx>{`
        ...
        .srs-suggested-actions {
          display: flex;
          gap: 8px;
        }

        /* First-time welcome - simpler */
        .srs-overview__welcome--first {
          text-align: center;
          padding: 48px 32px;
        }

        .srs-overview__welcome--first .srs-welcome-header {
          flex-direction: column;
          align-items: center;
          text-align: center;
          margin-bottom: 24px;
        ...
        .srs-btn--wizard:hover {
          opacity: 0.9;
        }

        .srs-btn--wizard-outline {
          background: transparent;
          border: 1px solid #ec4899;
          color: #ec4899;
        }

        .srs-btn--wizard-outline:hover {
