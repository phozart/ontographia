...
export function SRSProvider({ children }) {
  ...
  const { activeDomain, setActiveDomain } = useDomains();
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({ domainId: activeDomain });
  ...
  const loadSession = useCallback(async (sessionId) => {
    ...
    try {
      ...
      // If session is in a different domain, switch to that domain
      // This happens when opening a shared link directly
      const sessionDomainId = data.session?.domain_id;
      if (sessionDomainId && sessionDomainId !== activeDomain) {
        setActiveDomain(sessionDomainId);
      }
  ...
  }, [authHeaders, activeDomain, setActiveDomain]);
  ...
  const getParticipants = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/srs/sessions/${activeSession.id}/share`, {
        headers: authHeaders,
      });

      if (!res.ok) {
        console.warn('Failed to get participants:', res.status);
        return [];
      }
  ...
  const shareSession = useCallback(async (userId, sessionRole = 'viewer') => {
    if (!activeSession?.id || !userId) {
      setError('No active session or user ID');
      return null;
    }
    ...
    try {
      const res = await fetch(`/api/srs/sessions/${activeSession.id}/share`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders },
        body: JSON.stringify({ userId, role: sessionRole }),
      });

      if (!res.ok) {
        const errData = await res.json();
        throw new Error(errData.error || 'Failed to share session');
      }
      ...
      if (data.participant) {
        setParticipants(prev => [...prev.filter(p => p.userId !== userId), data.participant]);
      }
  ...
  const unshareSession = useCallback(async (userId) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errData = await res.json();
        throw new Error(errData.error || 'Failed to remove participant');
      }

      // Update local participants list
      setParticipants(prev => prev.filter(p => p.userId !== userId && p.user_id !== userId));
  ...
  const updateSessionVisibility = useCallback(async (visibility) => {
    if (!activeSession?.id) {
    ...
    try {
      ...
      setActiveSession(prev => prev ? { ...prev, visibility } : prev);
      setSessions(prev => prev.map(s =>
        s.id === activeSession.id ? { ...s, visibility } : s
      ));
  ...
  const searchUsers = useCallback(async (query, domainId = null) => {
    ...
    try {
      ...
      if (!res.ok) {
        console.warn('User search failed:', res.status);
        return [];
      }
  ...
  const getSessionShareUrl = useCallback(() => {
    if (!activeSession?.displayId && !activeSession?.display_id) return null;
    const displayId = activeSession.displayId || activeSession.display_id;
    return `${window.location.origin}/app/spaces/srs/session/${displayId}`;
  }, [activeSession]);
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (contentType && contentType.includes('application/json')) {
          ...
          const errorMsg = errData.details
            ? `${errData.error}: ${errData.details}`
            : (errData.error || 'Failed to update element');
          throw new Error(errorMsg);
        } else {
          throw new Error(`Server error: ${res.status} ${res.statusText}`);
        }
  ...
  const createConnection = useCallback(async (fromId, fromType, toId, toType, connectionType, note = '') => {
    ...
    try {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        ...
        console.error('Server returned non-JSON response:', res.status, text.substring(0, 200));
  ...
  }, [user, activeDomain, fetchSessions, resetSessionElements]);
