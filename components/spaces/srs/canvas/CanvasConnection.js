...
export default function CanvasConnection({
  ...
  const lineColor = color || typeConfig.color;
  ...
  // Arrow marker ID unique to connection type and color
  const colorHash = lineColor.replace('#', '');
  const markerId = `arrow-${connectionType}-${colorHash}`;
