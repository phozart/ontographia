// components/spaces/ba/BAWorkspace.js
...
// ============ TAB INFO FOR BREADCRUMBS ============
const TAB_INFO = {
  requirements: { id: 'requirements', name: 'Requirements', icon: DescriptionIcon, color: '#dc2626' },
  delivery: { id: 'delivery', name: 'Delivery', icon: ViewKanbanIcon, color: '#7c3aed' },
  trace: { id: 'trace', name: 'Traceability', icon: TimelineIcon, color: '#059669' },
  documents: { id: 'documents', name: 'Documents', icon: ArticleIcon, color: '#64748b' },
  map: { id: 'map', name: 'Strategy Map', icon: MapIcon, color: '#3b82f6' },
};

// ============ BA NAVIGATOR ============
// Sidebar navigation component following the WorkspaceLayout pattern
function BANavigator({
  activeProject,
  activeViewpoint,
  setActiveViewpoint,
  filteredArtefacts,
  ...
  return (
    <nav className="ba-navigator">
      {/* Header */}
      <div className="ba-nav-header">
        <div className="ba-nav-title">
          <DescriptionIcon className="ba-nav-icon" />
      ...
      {/* Viewpoint Indicator */}
      <div className="ba-nav-viewpoint">
        <ViewpointIndicator activeViewpoint={activeViewpoint} />
      </div>

      {/* Search */}
      <div className="ba-nav-search">
        <SearchBar
          value={searchQuery}
      ...
      {/* Repository Tree */}
      <div className={`ba-nav-tree ${sidebarCollapsed ? 'collapsed' : ''}`}>
        <div className="ba-nav-tree-header">
          <span className="ba-nav-tree-title">Repository</span>
          <button
            className="ba-nav-tree-toggle"
...
export default function BAWorkspace({ diagramComponent }) {
  ...
  // Build breadcrumbs based on current tab
  const tabInfo = TAB_INFO[activeTab];
  const breadcrumbsContent = activeTab ? (
    <Breadcrumbs>
      <Breadcrumb
        icon={DashboardIcon}
        label="BA Studio"
      ...
      <Breadcrumb
        icon={tabInfo?.icon}
        label={tabInfo?.name || activeTab}
        active
      />
    </Breadcrumbs>
  ) : null;

  // Build modals component
  const modalsContent = (
    <>
  ...
  const renderMainContent = () => (
    <div className="ba-workspace-content">
    ...
    <WorkspaceLayout
      navigator={
        <BANavigator
          ...
          relationships={relationships}
          selectedArtefact={selectedArtefact}
          onSelectArtefact={handleSelectFromTree}
