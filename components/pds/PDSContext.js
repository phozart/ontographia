...
export function PDSProvider({ children }) {
  ...
  const fetchArtefacts = useCallback(async (filters = {}) => {
    if (!user || !activePdsProject?.id) return;
  ...
  }, [user, activePdsProject?.id, authHeaders]);
  ...
  const fetchRelationships = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({ projectId: activePdsProject.id });
  ...
  }, [user, activeProject?.id, fetchPdsProjects]);
  ...
  }, [activePdsProject?.id, fetchStats, authHeaders]);
