      ...
      {filteredFrames.length === 0 ? (
        <div className="srs-space__empty srs-space__empty--guided">
          <div className="srs-empty__header">
            <div className="srs-empty__phase-badge" style={{ backgroundColor: '#10b981' }}>
              Explore Phase • Step 2
            </div>
            <FilterFramesIcon style={{ fontSize: 48, color: SRS_SPACES.frames.color }} />
            <h2>Examine Your Frames</h2>
            <p className="srs-empty__purpose">
              Frames are the invisible lenses through which we see problems. Surface the assumptions,
              beliefs, and mental models shaping your thinking — then challenge them.
            </p>
          </div>

          <div className="srs-empty__guidance">
            <h3>What Frames to Capture</h3>
            <p>Look for hidden beliefs that might be limiting your options.</p>

            <div className="srs-empty__prompts">
              <div className="srs-empty__prompt-group">
                <h4>Assumptions</h4>
                <ul>
                  <li>"We assume the budget is fixed..."</li>
                  <li>"We assume customers want..."</li>
                  <li>"We assume we can't change..."</li>
                </ul>
              </div>
              <div className="srs-empty__prompt-group">
                <h4>Constraints</h4>
                <ul>
                  <li>"We must use the existing..."</li>
                  <li>"Legal requires that we..."</li>
                  <li>"The deadline is immovable..."</li>
                </ul>
              </div>
              <div className="srs-empty__prompt-group">
                <h4>Mental Models</h4>
                <ul>
                  <li>"Success looks like..."</li>
                  <li>"The market works by..."</li>
                  <li>"Our competitors will..."</li>
                </ul>
              </div>
            </div>

            <div className="srs-empty__maturity-hint">
              <strong>Key question:</strong> What if this assumption is wrong? How would that change our options?
            </div>
          </div>
          ...
          <div className="srs-empty__next-step">
            <span>Next:</span> After examining frames, move to <strong>Parallel States</strong> to explore
            multiple possible futures simultaneously.
          </div>
        </div>
      ) : (
        <div className="srs-cards-grid">
          {filteredFrames.map(frame => {
            const typeConfig = FRAME_TYPES[frame.frame_type] || FRAME_TYPES.assumption;
            ...
            const cardColor = frame.is_challenged ? '#ef4444' : typeConfig.color;
